...
const safariData: Record<string, any> = {
  "taste-of-tanzania": {
    ...
    excluded: [
      ...
      "Tips (tipping guideline $15.00 pp per day)",
      "Personal items (souvenirs, travel insurance, visa fees, etc.)"
    ],
    activities: [
      "Balloon Safari ($500 per person)",
      "Maasai Village Visit ($50 per vehicle)",
      "Night Game Drive Tarangire or Lake Manyara ($120 per person)",
      "Ngorongoro Crater Rim Walk ($30 per person)",
      "Visit to the Olduvai Gorge and Museum ($40 per person)"
    ]
  },
    ...
    itinerary: [
      {
        ...
        title: "DAY one",
        activities: [
          { time: "MORNING", description: "Pick up from your lodge in Arusha" },
          { time: "DAY", description: "Drive to Tarangire National Park for game viewing" },
          { time: "EVENING", description: "Return to lodge for dinner and overnight" }
        ]
      }
    ],
    included: [
      ...
      <section className="py-16 bg-muted/30">
        <div className="container mx-auto px-4 max-w-5xl">
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            <div>
              <h1 className="text-4xl md:text-5xl font-serif uppercase text-secondary mb-4">
                {safari.name}
              </h1>
        ...
        <div className="container mx-auto px-4 max-w-5xl">
          ...
          <div className="mb-16 grid md:grid-cols-2 gap-8">
            <div>
              ...
              <ul className="space-y-2">
                {safari.included.map((item: string, idx: number) => (
            ...
            <div>
              ...
              <ul className="space-y-2">
                {safari.excluded.map((item: string, idx: number) => (
                  <li key={idx} className="flex items-start gap-2">
                    <span className="text-primary mt-1">•</span>
                    <span>{item}</span>
                  </li>
          ...
          {safari.activities && safari.activities.length > 0 && (
            <div className="mb-16">
              ...
              <ul className="space-y-2">
                {safari.activities.map((activity: string, idx: number) => (
                  <li key={idx} className="flex items-start gap-2">
                    <span className="text-primary mt-1">•</span>
                    <span>{activity}</span>
                  </li>
